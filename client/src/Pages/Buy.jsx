    ...
    return (
      <>
        <div className="container mx-auto mt-24">
          <h1 className="font-serif text-7xl text-center font-medium text-primary pb-6">Ваши покупки</h1>
          <hr className="w-4/5 mx-auto mt-2 border-b-2 border-primary" />
          <div className="mx-5 sm:mx-0 mt-8">
            <h2 className="text-2xl text-primary font-bold mb-4">Список покупок</h2>
            <ul>
              {BuyOperations.map((operation, index) => {
                ...
                return (
                  <li key={index}>
                    <div className="flex py-5 justify-between mb-2">
                      ...
                      <div className={operation.status === 'Отменен' ? 'text-green text-base sm:text-xl font-semibold' : 'text-red text-base sm:text-xl font-semibold'}>
                        {operation.amount}₽ ({operation.status})
                      </div>
                    </div>
                    {index < BuyOperations.length - 1 && (
